...
export class EmailTemplateRepositoryMySQL extends BaseEmailTemplateRepository {
  async findAll(): Promise<EmailTemplate[]> {
    const [rows]: any = await MysqlConnection.execute(
      'SELECT id_template AS id, asunto AS subject, cuerpo AS body, layout_html AS layoutHtml, id_tipo_correo AS typeId FROM template_correo'
    );
    return rows;
  }
  ...
  async findById(id: number): Promise<EmailTemplate | null> {
    const [rows]: any = await MysqlConnection.execute(
      'SELECT id_template AS id, asunto AS subject, cuerpo AS body, layout_html AS layoutHtml, id_tipo_correo AS typeId FROM template_correo WHERE id_template = ?',
      [id]
    );
  ...
  async create(template: Omit<EmailTemplate, 'id'>): Promise<EmailTemplate> {
    const [result]: any = await MysqlConnection.execute(
      'INSERT INTO template_correo (asunto, cuerpo, layout_html, id_tipo_correo) VALUES (?, ?, ?, ?)',
      [template.subject, template.body, template.layoutHtml ?? null, template.typeId]
    );
  ...
  async update(id: number, template: Omit<EmailTemplate, 'id'>): Promise<EmailTemplate | null> {
    await MysqlConnection.execute(
      'UPDATE template_correo SET asunto = ?, cuerpo = ?, layout_html = ?, id_tipo_correo = ? WHERE id_template = ?',
      [template.subject, template.body, template.layoutHtml ?? null, template.typeId, id]
    );
